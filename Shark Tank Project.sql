...
SELECT COUNT([Pitchers Gender]) FROM [Shark Tank dataset]
WHERE [Pitchers Gender] = 'Female'
SELECT COUNT([Pitchers Gender]) FROM [Shark Tank dataset]
...
SELECT 
[Gender Ratio] = (SELECT CAST(SUM([Pitchers Gender]) AS FLOAT) FROM [Shark Tank dataset]
...
SELECT COUNT(DISTINCT([Startup Name])) 
FROM [Shark Tank dataset]
WHERE [Shark Tank dataset].[Got Deal] = 1

SELECT COUNT([Pitchers Gender])
FROM [Shark Tank dataset]
WHERE [Shark Tank dataset].[Got Deal] = 1 
AND [Pitchers Gender] = 'Female'

SELECT COUNT([Pitchers Gender])
FROM [Shark Tank dataset]
WHERE [Shark Tank dataset].[Got Deal] = 1 
AND [Pitchers Gender] = 'Male'

SELECT COUNT([Pitchers Gender])
FROM [Shark Tank dataset]
WHERE [Shark Tank dataset].[Got Deal] = 1 
AND [Pitchers Gender] = 'Mixed Team'

/* SUCCESS RATE OF GETTING THE DEAL*/????????????????????????????????????????????????????????????

SELECT
b.Tot_Contestants/Got_grant AS Success_Rate_Percentage
FROM
(
	SELECT 
	COUNT(([Startup Name])) AS [Tot_Contentstants] FROM [Shark Tank dataset]
		FROM(
			(SELECT 
			COUNT(([Startup Name])) AS [Got_grant]
			FROM [Shark Tank dataset]
			WHERE [Got Deal] = 1
			)
		)a
...
CREATE TABLE #SuccessRatePercentage
(
	Tot_Contestants INT,
	Got_Grant INT,
	S_Percentage INT
...
INSERT INTO #SuccessRatePercentage
(
	Tot_Contestants,
	Got_Grant,
	S_Percentage
...
SELECT 
	COUNT(([Startup Name])) AS [Tot_Contestants],
	Got_Grant = CASE WHEN [Got Deal] = 1 THEN 1 ELSE 0 END,
	S_Percentage = Got_Grant/Tot_Contestants
FROM [Shark Tank dataset]
WHERE Got_Grant = 1

SELECT * FROM #SuccessRatePercentage
...
AVG(CAST([Guest Investment Equity] AS FLOAT))
FROM [Shark Tank dataset] 
WHERE [Guest Name] IS NOT NULL AND [Guest Investment Amount] IS NOT NULL AND [Guest Investment Equity] IS NOT NULL
GROUP BY [Guest Investment Amount],[Guest Investment Equity], [Guest Name]
ORDER BY [Guest Name]
...
SELECT 
[Pitchers City],
COUNT([Pitchers City]) AS City_Count
FROM [Shark Tank dataset]
GROUP BY [Pitchers City]
ORDER BY City_Count
ON a.[Startup Name] = [b.Startup Name]
